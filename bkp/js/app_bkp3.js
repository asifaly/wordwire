var wordWire = angular.module('wordWire',['firebase']);
       wordWire.constant('FIREBASE_URI', 'https://wordwire.firebaseio.com/');
        wordWire.controller('WordCtrl', ['$scope', 'WordsService', '$firebase', 'FIREBASE_URI', function ($scope, WordsService, $firebase, firstlet, FIREBASE_URI) {
            ...
            var a = $scope.newword.name;
            var b = $scope.newword.name.charAt($scope.newword.name.length-1);
            WordsService.addStats({lastword: a, firstletter: b});
            //to store and get firstletter & lastword
            ...
            //$scope.wordpattern = firstlet($scope.words(4).name);
    ...
    var stat = new Firebase(FIREBASE_URI+"/stats");
    var stats = $firebase(stat.limit(1));
    var last = new Firebase(FIREBASE_URI+"/stats/lastword");
    var lasts = $firebase(last).$asObject();
    var first = new Firebase(FIREBASE_URI+"/stats/firstletter");
    var firsts = $firebase(first).$asObject();
